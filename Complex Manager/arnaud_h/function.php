...
if (is_numeric($real_number) && is_numeric($imaginary_number))
{
    ...
    if (($real_number <= '10' && $real_number >= '-10') && ($imaginary_number <= '10' && $imaginary_number >= '-10'))
    {
        ...
        echo "<li><u>Argument (arg(z)) :</u></li>";
	$test = $real_number/$module;
	echo "<li>cos θ : " . round($test, 6)  . "</li>";
	$test2 = $imaginary_number/$module;
	echo "<li>sin θ : " . round($test2, 6)  . "</li>";
        echo "<li><u>Ecriture trigonométrique :</u> " . round($module, 6) . "(cos " . round($test, 6) . " + isin " . round($test2, 6) . ")</li>";
        echo "</ul>";
    }
    else
